...
export const mapVitneboks = (vitneboksRaw: any) => {
  ...
  const questions = vitneboksRaw!.questions
    ? (
        Object.entries(vitneboksRaw.questions).map(
          ([id, q]: [string, any]) => ({ ...q, id }),
        ) as Question[]
  ...
  return {
    ...
    questions,
    completedVideoIds: vitneboksRaw?.completedVideoIds
      ? Object.values(vitneboksRaw?.completedVideoIds)
      : [],
...
export const isPhoneDevice = (): boolean => {
  ...
  return /iphone|ipod|android.*mobile|blackberry|windows phone|opera mini|mobile/i.test(
    userAgent,
  );
};

// Save recording completion timestamp and question to localStorage
export const saveRecordingCompletion = (
  vitneboksId: string,
  question: Question,
